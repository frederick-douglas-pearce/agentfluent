...
| Web search ($10 / 1k) | C (surcharge) | `compute_cost(surcharge_usd=…)` (#539) | Medium *if used* | Overlay — future (#539, blocked by #547; counts present in JSONL) | **Landed upstream** in `genai-prices==0.1.4` as `web_searches_kcount` (populated on 16 of 21 Anthropic models) — [pydantic/genai-prices#288](https://github.com/pydantic/genai-prices/pull/288). There is **no local overlay to retire** here — `compute_cost(surcharge_usd=…)` has no production caller, so web-search cost is unpriced today; #539 now scopes to reading the upstream rate rather than building the overlay first. |
