...
AgentFluent prices sessions on top of [pydantic/genai-prices](https://github.com/pydantic/genai-prices) (MIT). For Anthropic at the pinned `0.1.4` it models `input_mtok`, `output_mtok`, `cache_read_mtok`, `cache_write_mtok` (5m-equivalent), `cache_write_1h_mtok`, `web_searches_kcount`, context-length `tiers`, and dated `constraint`s. Two of those — `cache_write_1h_mtok` and `web_searches_kcount` — arrived with the `0.1.4` bump (#661); the others predate it. Their rows below therefore record **upstream** status rather than a retirement, and the two are at different stages locally: the 1h cache write has a landed local overlay (#534) that stays until #662 retires it onto the upstream key, while web search has **no local overlay yet** — the adapter does not read `web_searches_kcount`, so web-search cost is simply unpriced until #539. The levers it does **not** model — which AgentFluent must supply via a local pricing overlay, and/or request upstream — are our coverage ledger. Each lever plugs into the [base ⊕ overlay merge seam](#the-base--overlay-merge-seam-547) at one of three named plug points (its **Class** below):

| Gap | Class | Seam plug point | Cost impact for users | Local status | Upstream (genai-prices) |
|---|---|---|---|---|---|
| 1-hour cache write (2×) | A (rate-level) | `ModelPricing.__post_init__` (derived 2× input) | **High** (commonly the dominant TTL) | Overlay — landed (#534): parser splits `usage.cache_creation` into 5m/1h, priced separately | **Landed upstream** in `genai-prices==0.1.4` as `cache_write_1h_mtok` (populated on 19 of 21 Anthropic models) — [pydantic/genai-prices#295](https://github.com/pydantic/genai-prices/issues/295). The local overlay **stays in place** until #662 retires it onto the upstream key (verified price-neutral: upstream is exactly 2× `input_mtok` on every model that carries the key). |
...
| Web search ($10 / 1k) | C (surcharge) | `compute_cost(surcharge_usd=…)` (#539) | Medium *if used* | Overlay — future (#539, blocked by #547; counts present in JSONL) | **Landed upstream** in `genai-prices==0.1.4` as `web_searches_kcount` (populated on 16 of 21 Anthropic models). Note it did **not** arrive via [pydantic/genai-prices#288](https://github.com/pydantic/genai-prices/pull/288), which was closed unmerged on 2026-08-27 and proposed a differently-named key; the shipped key reached the dataset by another route. There is **no local overlay to retire** here — `compute_cost(surcharge_usd=…)` has no production caller, so web-search cost is unpriced today. #539 is unchanged as of this writing and still scoped to building the overlay; re-scoping it onto the upstream key is that issue's call, not this row's. |
