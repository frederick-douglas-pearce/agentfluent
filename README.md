...
- **CLI-native and local by default.** `agentfluent analyze --format json | jq ...` fits agent developer workflows (terminal, CI/CD, PR checks). No outbound network calls unless you explicitly opt in via `--github` (Tier 3 GitHub enrichment); no cloud services, no API key required.
...
| **API calls** | Anthropic API for LLM-as-judge scoring | None by default; `--github` opts in to GitHub-API quality signals via the local `gh` CLI |
...
agentfluent analyze --project codefluent --since 7d            # Sessions from the last 7 days only
...
agentfluent analyze --project codefluent --git                 # + Tier 2 local-git quality signals
agentfluent analyze --project codefluent --github              # + Tier 3 GitHub-API quality signals (opt-in)
...
- **Quality**: parent's mid-flight corrections (`USER_CORRECTION`), file rework density (`FILE_REWORK`), and reviewer-caught rate with `parent_acted` attribution (`REVIEWER_CAUGHT`). Recommendations carry a `[quality]` axis label and per-recommendation `axis_scores` annotations.
- **Quality Tier 2 (opt-in via `--git`)**: local-git `FEAT_FIX_PROXIMITY` — pairs `feat:` commits with subsequent `fix:` commits on shared files and correlates back to whether the originating session used a review-style subagent.
- **Quality Tier 3 (opt-in via `--github`)**: GitHub-API signals `CI_FAILURE_FIRST_PUSH` (PRs whose first commit's combined CI status is `failure`/`error`) and `PR_REVIEW_COMMENT_DENSITY` (external review-comment density above the configured threshold). Requires the `gh` CLI for auth, a file-backed response cache trims repeated GitHub calls, and a first-run consent prompt records opt-in under `~/.config/agentfluent/`. The JSON envelope exposes `tier3_degraded: bool` so CI consumers can see when rate limits forced a partial fetch. **Zero Tier 3 signals on a clean `--github` run is a healthy outcome, not a broken feature** — a "push only on green CI" workflow or solo-author PRs legitimately produce no findings; `tier3_degraded` (not signal count) is the load-bearing "did the enrichment run" flag. See the [`CI_FAILURE_FIRST_PUSH`](docs/GLOSSARY.md#ci_failure_first_push) and [`PR_REVIEW_COMMENT_DENSITY`](docs/GLOSSARY.md#pr_review_comment_density) glossary entries for the per-signal healthy-silence patterns.

Above the Recommendations table, a **Top-N priority fixes summary** ranks findings by a composite `priority_score` that combines severity, occurrence count, cost impact (for `target='model'` mismatches), trace-evidence boost, and quality-evidence boost — so the highest-leverage changes surface first instead of asking the reader to scan a flat severity-sorted list. The sort key is part of the JSON envelope (`aggregated_recommendations[].priority_score`), so a CI gate can fail the run on priority regression. An **Offload Candidates** section calls out clusters of repeating tool-use patterns in the parent thread and proposes subagent / skill drafts that move that work onto cheaper-tier models — the dominant cost lever for users running agents at scale.
...
**Schema v2 additions (v0.11, additive — no version bump):** each entry in `data.sessions` carries `session_kind: "sdk" | "cli" | "unknown"` — whether the session was produced by an Agent SDK run, Claude Code interactive, or is indeterminate — alongside `entrypoint: string | null`, the raw runtime value it was derived from (`"sdk-py"`, `"cli"`, or a future/unrecognized value). Both are published because they answer different questions: an *unrecognized* runtime classifies as `"unknown"` while `entrypoint` still reports it verbatim, so a Claude Code vocabulary change surfaces as a readable value rather than an unexplained `unknown`. The pair is per-session, never aggregated at the top level — one project directory routinely mixes co-located SDK and Claude Code sessions, which is exactly the ambiguity these fields resolve. `agentfluent diff` does not consume either field, so baselines from either side of v0.11 compare unchanged. The table view shows the same classification: a composition line in the footer (`Sessions analyzed: 76 (24 Claude Code, 52 SDK)`), an `[SDK]` badge when `--session` scopes the run, and a `Kind` column in the `--verbose` per-session breakdown.
...
AgentFluent is designed so data stays on your machine. The attack surface is small by construction — no web server, no HTML rendering, no webview, and **no outbound network calls unless you explicitly opt in via `--github`** (Tier 3 GitHub enrichment). This table summarizes the layers that protect it:
