...
## D050: S0 dogfood-runner — local cron scheduling (decoupled from #451), deterministic anti-false-green gate

**Date:** 2026-07-06
**Context:** Building S0 (#590) surfaced two design questions the PRD/epic left open. (1) **Scheduling.** The #590 DoD requires the runner scheduled from day one "same scheduling surface as the #451 research-scout cron" — but #451's mechanism is *undecided* (D037 deferred it: Claude Code `schedule` skill vs GitHub Actions vs local cron) and no cron artifact exists, so S0 had to *establish* a surface, not copy one. (2) **The AC "surfaces CLI exit codes rather than swallowing them"** conflicts with the literal "each subagent drives the `analyze` CLI" AC: routing the pass/fail signal through an LLM subagent is fragile (it can transcribe an exit code wrong or silently retry-and-report success — PRD Risk R). Architect review on #590 flagged both.

**Decisions:**
- **Local cron, decoupled from #451.** S0's constraints are strictly tighter than the scout's: it reads the **local** corpus (`~/.claude/projects/`), needs local auth, and writes sessions back locally — so a cloud/schedule-skill routine (which runs server-side and cannot see the local corpus) and GitHub Actions (same blind spot) are **both eliminated for S0 regardless of #451**. Ship a repo-tracked, idempotent local-cron installer (`tools/dogfood_runner/install-cron.sh`, default 12:30 daily) and keep the runner scheduler-agnostic (a plain `uv run -m` entrypoint) so the mechanism can change later. #451 stays open for the scout, whose web-based data leaves all three mechanisms viable.
- **Deterministic, code-aware gate owns pass/fail; subagents are narrative-only.** The parent shells out to the real CLI in plain Python (`cli_runner.py`) and reads `returncode` directly — no LLM on the correctness path. The exit mapping is **code-aware**, not `== 0`: `analyze` exit 2 (NO_DATA) is a benign empty per-slug window, exit 1 is the real red, `diff` exit 3 is a *regression finding* surfaced (not a runner error). Process exit reflects analysis health only (regressions are reported, not failed on). The Haiku subagents (under an Opus parent `query()`) synthesize the narrative only; synthesis failure never flips the gate. This deviates from the literal "subagent drives the CLI" AC wording, by design, to satisfy the "surface exit codes" AC — flagged to PM.
- **Location + state.** Runner lives at `tools/dogfood_runner/` (not throwaway `research/`, not `examples/` which reads "copy me") and doubles as the documented canonical example SDK agent. Snapshots persist under `$XDG_STATE_HOME/agentfluent/dogfood/` (user-global, out-of-tree) — never committed (absolute paths + project data; in-tree = one `.gitignore` slip from leaking) and never auto-written into the curated `.claude/specs/analysis/` dogfood dir. Bounded rolling window default `7d` via the existing `--since` surface (a build-time tuning value, not a filing decision — robust to a sporadically-worked corpus and to missed cron days, since consecutive windows must overlap for the diff); window-over-window `diff` dogfoods the regression surface. Parent-Opus/child-Haiku routing emits the model-divergence + nested-trace bytes S5 (#595) and #112 will consume.

**Rationale:** The "same surface as #451" instruction was under-specified given #451 is open and its premise (all three mechanisms viable) doesn't hold for a local-corpus reader — so decoupling and picking the only viable mechanism now is correct, not scope creep. The anti-false-green gate is the load-bearing correctness guarantee: a broken analysis must never report green, which only deterministic `returncode` reading can ensure. Keeping the SDK import lazy (in `runner.py`) means the gate + unit tests run without the `research` dependency-group, and the deterministic core is fully testable with the CLI mocked.
...
## D055: Every non-excluded field on a serialized model is public JSON API by default — `#591`'s `session_class` became public unintentionally and was renamed to the ratified `session_kind` before release

**Date:** 2026-07-17
**Context:** `#592` (Story S2 of the Agent SDK ingestion epic `#589`) specifies that `analyze --format json` emit **both** `session_kind: "sdk"|"cli"|"unknown"` and the raw `entrypoint`. That name was ratified 2026-07-06 in the `#592` AC and `prd-v0.11.md` §4/§10. Three days later `#591` (S1) landed the classification result on `SessionAnalysis` as **`session_class`**, and `#112` consumed it — so by the time S2 was picked up, the code shipped a *differently-named* field with the ratified field's exact semantics. `SessionAnalysis` is serialized wholesale (`AnalysisResult.model_dump(mode="json")` in `cli/commands/analyze.py`), so `session_class` was **already being emitted** at `data.sessions[i].session_class` under `SCHEMA_VERSION = "2"`.
**Decisions:**
- **The general rule (the reason this entry exists): a field added to a wholesale-serialized model is public JSON contract the moment it lands, whether or not anyone intended it.** `session_class`'s own docstring justified it as *internal* diagnostics gating and never mentioned the JSON contract — yet it reached the public envelope purely as a `model_dump()` side effect. Therefore: **name fields on serialized models for the public contract, check the name against the ratified spec before landing, and use `exclude=True` for genuinely internal fields** (the precedent already set by `SessionAnalysis.messages`).
- **Rename `session_class` → `session_kind` (PM ruling option (c); maintainer-approved).** The drift is in the *code*, not the AC: `session_kind` was ratified first, and `#591`'s naming latitude was explicitly scoped to `entrypoint` on the *message* model, not to the session field's public JSON name. Rejected: (a) accept `session_class` + reframe the AC — that rewrites ratified wording to match an accident, and is the option that would have needed maintainer sign-off; (b) emit both names — two public names for one value, forever, to avoid a one-week-old zero-user rename.
- **The rename is free, and this was the last moment it would be.** Verified: latest release is **v0.10.0** (2026-07-03) and `session_class` landed 2026-07-09/07-14 — `git show v0.10.0:src/agentfluent/analytics/pipeline.py | grep -c session_class` → **0**. The field is in **zero PyPI artifacts**; no installed consumer can break, and the sole consumer (`#112` `model_routing.py`) is in-repo and in the same unreleased milestone. Shipped as an **additive** change (D017): no envelope bump, no `BREAKING CHANGE:` footer — contrast D029, which applied to genuinely *released* behavior.
- **`session_kind` and raw `entrypoint` are both published and are NOT redundant.** Both derive from one new `select_entrypoint(messages)` precedence, with `classify_session` a thin function over it — so a `session_kind: "sdk"` beside an `entrypoint: "cli"` is *unrepresentable by construction* (the "first non-None" shape the architect blocked would have permitted exactly that self-contradiction). The pairing's value: an **unrecognized** runtime classifies as `unknown` while `entrypoint` still reports the value verbatim, turning the `prd-v0.11.md` §9 entrypoint-drift risk into a self-describing report instead of a dead end.
**Rationale:** The recurring failure this guards is a naming decision made at the *implementation* altitude silently overriding one ratified at the *spec* altitude, made invisible because the serialization is implicit — nobody chose to publish `session_class`; a `model_dump()` did. Fixing it cost one mechanical rename only because it was caught pre-release; the same oversight found after v0.11 would have been permanent. The generalizable half (fields on serialized models are public by default) has teeth well beyond `#592`: the next internal field added to `SessionAnalysis` ships publicly whether anyone intends it or not.
**Reference:** issue `#592` (ratified ACs), architect review (`#592` comment 5000544824), PM ruling + human plan gate (release-loop ledger `.claude/loop/v0.11.0/issue-592.plan.md`); source `#591`/S1 (`classify_session`, `entrypoint` primitive), `#112` (first consumer), `prd-v0.11.md` §4/§9/§10; related D009 (0.x expect-breaking-changes; versioned+documented JSON schema is a *1.0* criterion), D013 (`entrypoint` discriminator), D017 (additive fields don't break consumers), D029 (released breaking change → CHANGELOG note + bump).

---
## D056: `toolUseResult.totalTokens` is exclusive of child agents **and** is a final-turn context proxy, not cumulative spend — `#595` ratifies the aggregation convention and defers the spend defect it exposed

**Date:** 2026-07-18
**Context:** `#595` (Story S5 of the Agent SDK ingestion epic `#589`) carries a named sub-task: settle whether a parent invocation's `totalTokens` already includes its children's tokens *before* any aggregation sums them, and encode the resolved convention so no consumer double-counts. The question was discharged empirically at plan time (release-loop step 3) against the live corpus — ~1036 subagent trace files, 683 rollups carrying both a parent `toolUseResult.totalTokens` and the child's own trace — rather than deferred to implementation.
**Decisions:**
- **(A) — RATIFIED: `totalTokens` is EXCLUSIVE of child agents.** In the live depth-2 capture the level-1 delegator's rollup reads `totalTokens = 5495` while its depth-2 child spent `3925`; the child's tokens appear nowhere in the parent's figure, additively or otherwise. **Therefore the multi-level linker introduces no parent/child double-counting via `totalTokens`,** and an aggregate summing an invocation alongside its descendants is not double-counting on that axis. This is the convention `#595` AC#3 asked for.
- **(B) — RECORDED, DEFERRED: `totalTokens` is not cumulative spend at all.** It equals the agent's **final assistant turn** usage (`input + output + cache_creation + cache_read`). Re-verifiable counts: **575/683 (84%) match the final turn exactly; 0/683 match the sum of the agent's turns;** the remaining 108 sit within ~1–3% of the final turn and run **10–40× below** the sum (e.g. `totalTokens` 156,505 · final turn 155,486 · sum of 59 turns 4,751,903). The figure is a **final-turn context-size proxy** — neither tokens billed nor tokens processed — because every turn re-reports the whole context it was given, so summing turns counts the same conversation prefix repeatedly. Measured over 397 multi-turn live traces, the recurring component is `cache_creation` and plain `input_tokens` in **100%** of traces and `cache_read` in **97%** (92% monotonically growing); *which* component carries it depends on cache state, so the durable claim is the re-reporting, not any one field. (The `nested_session` fixture is cold-cache and carries the recurrence in `cache_creation`.)
- **The ~12 sites that sum this field are deliberately NOT changed by `#595`** (PM scope ruling (a), maintainer-approved). Sharpest instance: `agent_token_percentage` divides a sum of **final-turn context sizes** by `token_metrics.total_tokens`, a sum of **per-turn usage components** — different units, so the published ratio is not meaningfully bounded and can exceed 100%. `estimated_total_cost_usd` multiplies the same non-spend numerator by a blended rate. This is a pre-existing defect (v0.3 / `#111` era), **not** a regression `#595` introduces, and its fix is *enabled* by S5's cross-file linker (real spend is computable by summing a child trace's per-turn `usage`) — so bundling the fix into `#595` would invert the dependency.
- **One entry covers both halves, by design.** Recording (A) without (B) would leave a decision record that is technically true and practically misleading: a future reader asking "is `total_tokens` safe to sum?" gets the complete answer here or gets a half-truth. The `AgentInvocation.total_tokens` docstring states both facts at the site where the next consumer will actually read them.
- **No rename, no runtime guard.** Unlike `session_class` under D055 — which had **zero** released artifacts — `total_tokens` **has shipped**, so **D029** governs: a rename would be a released breaking change, out of scope for an additive milestone. The convention is encoded as documentation (this entry + the field docstring), not as a validator.
- **Release-narrative constraint for v0.11.0.** The milestone's cost-correctness work concerns **rates** — the genai-prices base ⊕ overlay seam, date-aware lookup, and the tier gap split to `#639` — i.e. the price-per-token half. (B) concerns **quantities**. v0.11.0 release notes may claim improved *rate* correctness and must make **no claim** about per-agent cost accuracy or token attribution; a limitation note lands in `docs/COST_MODEL.md` via `#549` (D054 precedent for routing this class of caveat there).
**Rationale:** The generalizable lesson is that a field's *name* is not its *semantics*, and an SDK-supplied rollup named `totalTokens` invites exactly the reading the data refutes — every consumer that summed it read "total" as cumulative. Discharging the falsifier at plan time rather than after implementation is what surfaced this: the asked question (child inclusivity) had a clean answer that would have shipped alone and looked complete, while the unasked question sitting one layer beneath it silently governs two user-facing published numbers. D054 set the disposition precedent — reframe the issue to the narrow provable thing, split the investigation's larger find to its own issue with the evidence attached — and the empirical case against parking it in the backlog is `#143`, an unmilestoned cost-accuracy issue open since 2026-04-22.
**Reference:** issue `#595` (AC#3), architect review (`#595` comment), PM scope ruling + human plan gate (release-loop ledger `.claude/loop/v0.11.0/issue-595.plan.md`); corpus method and counts reproduced above for re-verification; `agent-sdk-session-format-findings.md` §4 (the depth-≥2 "no rollup" row this pass confirmed on live bytes); related D017 (additive fields), D029 (released breaking change → bump + CHANGELOG), D042 (hold scope at the epic), D054 (`#546` → `#639` split precedent + `COST_MODEL.md` routing), D055 (serialized fields are public API by default); downstream `#143` (per-invocation token split — its `toolUseResult.usage` premise is undercut by (B)).

---

## D057: The `totalTokens` defect is a deterministic 100% rule, not an 84% heuristic — corrected magnitude is ~15x silent **understatement**, superseding D056(B)'s characterization and retracting its ">100%" claim

**Date:** 2026-07-18
**Context:** `#646` was split out of `#595` with its evidence recorded in D056. A follow-up investigation re-derived that evidence against the live corpus (`~/.claude/projects`, 2026-07-18; 1,047 subagent trace files with ≥1 assistant turn, 691 linked rollups, 163 sessions) and both sharpened the rule and refuted one of D056's claims. Full write-up: `docs/TOKEN_ACCOUNTING.md`.
**Decisions:**
- **Scope of supersession — narrow.** D056 **(A)** (`totalTokens` is exclusive of child agents) **stands unchanged**; `#595`'s ratified aggregation convention is unaffected. Only D056(B)'s *characterization* and its ">100%" clause move.
- **(1) The rule is deterministic.** `totalTokens == usage.input_tokens + usage.output_tokens + usage.cache_creation_input_tokens + usage.cache_read_input_tokens` over the sibling `toolUseResult.usage` object — **691/691 (100%) exact**. D056(B)'s "final assistant turn, 84%" is **superseded as the characterization** (it is a *consequence*, not the rule) and retained only as a secondary observation at **582/691 (84.2%)**.
- **(2) This closes D056's open question (c).** The 16% non-final-turn cohort needs no special treatment: those are cases where the turn snapshotted into `usage` is not the trace's last assistant line. Same rule.
- **(3) The invariant is asserted as a test, not treated as a contract.** Anthropic documents the session JSONL format as internal and subject to change on any release ("scripts that parse these files directly can break on any release"). A format change must fail loudly rather than silently re-corrupt aggregates.
- **(4) RETRACTED: the ">100%" claim.** D056's "the published ratio is not meaningfully bounded and can exceed 100%" is **unsupported by measurement**. Across all 163 sessions with linked traces, `agent_token_percentage` reads median **1.4%**, max **31.6%**, **zero above 100%** — `fold_subagent_metrics_in` (`analytics/pipeline.py:258`) already folds real per-turn subagent usage into the denominator, so trace-linked invocations are understated at *both* ends. (Not strictly impossible: a trace-missing invocation adds to the numerator but not the denominator; per `#468` that configuration is confined to one historical session.)
- **(5) The real failure mode is silent understatement**, and it is **strictly worse** than the one D056 anticipated: a plausible 0.5% believed when the truth is 4.0%; a believable $1.82 when the truth is $18.69. An absurd number is self-disclosing; a plausible one is not. D056's remediation instinct was calibrated to the wrong failure mode.
- **(6) Quantified impact.** Six heaviest agent sessions: **$6.94 published vs $105.63 true — 15.2x**. Token-quantity ratio across 691 rollups: median **5.8x**, p90 **14.2x**, max **79.7x**. Direction is **always understatement**.
- **(7) Cost error (15.2x) exceeds token error (5.8x) because two errors compound:** a snapshot numerator *and* a cache-diluted `blended_rate` (`session_cost / session_total_tokens`, whose denominator counts cache reads at full token weight though they bill at 0.1x). **Any fix must correct the rate as well as the quantity.**
- **(8) Why it hid for four minor versions.** Summed across a run, the expensive components alone (`input + output + cache_creation`, excluding cache reads) come to a median **1.02x** of `totalTokens` (p90 1.21x) — each context token is cache-written roughly once per run, so `Σ cache_creation` ≈ final context size ≈ `totalTokens`. The rollup lands within a few percent of a real, meaningful quantity. That near-identity is why it propagated into published reference documentation.
- **(9) Measurement methodology — audit trail.** The magnitudes first recorded on `#646` (28.4x cost, 12.5x median tokens, 41.6% orphan) were **overstated ~2x by an ad-hoc script that did not deduplicate streaming snapshots**. Claude Code emits multiple `assistant` lines per logical turn sharing a `message.id`, each carrying a running `usage` snapshot; **986/1,047 (94%)** of trace files are affected and naive summing inflates **1.99x**. `core/parser.py` was never affected — the corrected method matches it on **80/80** sampled traces, and CodeFluent's independently measured corpus total agrees to **0.13%**. The figures in (6) are the corrected set.
- **(10) `#646` remains OUT of v0.11.0** (PM ruling, 2026-07-18). v0.11.0's cost work is the *rate* half; landing a quantity correction in the same release makes a composite ~15x movement unattributable and defeats independent validation against the Usage & Cost Admin API. The fix is undesigned, the orphan cohort's root cause is not yet decomposed, and `#595` (the enabling linker) is itself still open. **D042 governs.** `#646` becomes the headline of v0.12.0.
- **(11) SUPERSEDES D056's release constraint.** "Make no claim" is a constraint on *release notes* and does not reach a user running the CLI. Because v0.11.0 legitimately improves *rate* correctness while the composite `estimated_total_cost_usd` is **no more accurate than v0.10.0**, silence combined with a true rate claim **misleads by implicature**. v0.11.0 therefore ships an **explicit user-visible caveat** in `docs/COST_MODEL.md`, `README.md`/`docs/GLOSSARY.md`, and a CHANGELOG **"Known limitations"** block (routed via `#549`). Release notes may claim improved *price-per-token* correctness; they must **not** use unqualified "cost correctness" or "more accurate costs". A CLI/JSON runtime caveat is deliberately **not** added in v0.11.0 — it ships in v0.12.0 stating the corrected value and its coverage.
- **(12) The orphan / depth-≥2 finding is split to its own issue.** **30.7%** of all subagent processed tokens (213 of 1,047 trace files, 179,794,526 tokens) sit in traces with no parent rollup and are silently dropped (`analytics/pipeline.py:494`). Different root cause (*link coverage*, not *field semantics*) and larger blast radius: it corrupts `token_metrics` → session `total_cost`, not only per-agent attribution. **First AC: decompose the 30.7% into depth-≥2 (fixable by `#595` PR B) vs orphaned/rotated parents (likely not fixable, therefore a disclosure problem).**
- **(13) `#143` is superseded, not re-scoped.** Its `toolUseResult.usage` premise is the same one-turn snapshot; as written it would swap one non-spend number for four.
- **(14) No rename.** `total_tokens` has shipped, so **D029** governs. Corrected spend lands beside it; the field is re-documented as a context-size proxy.
- **(15) Admin API reconciliation is a one-time manual validation**, recorded in the v0.12 issue and `docs/TOKEN_ACCOUNTING.md` — **not** an automated AC and not a shipped feature. As a runtime dependency it needs org-scoped credentials and outbound egress, conflicting with AgentFluent's local-first posture.
**Rationale:** An empirical characterization recorded at the confidence level it was measured at will be *read* as the underlying rule. D056 recorded "84% match the final turn" — true, and it concealed a 100% deterministic invariant one layer beneath, which in turn concealed the correct failure mode. The 84% framing implied a fuzzy statistical artifact tolerable to defer; the 100% rule reveals a systematic 15x understatement in the product's central claim. **Re-derive before deferring: the strength of a finding's characterization determines the strength of the disposition it can justify.** Two corollaries. First, the ">100%" clause was reasoned from unit-mismatch rather than measured, and measurement refuted it — *a plausible mechanism is not evidence*. Second, (9) is the same lesson turned on this investigation: the first-pass magnitudes were themselves an unvalidated measurement, and only cross-checking against the production parser caught a 2x error before it hardened into this record.
**Reference:** issue `#646` (+ investigation comments 2026-07-18), `docs/TOKEN_ACCOUNTING.md` (PR `#647`), PM scope ruling (2026-07-18); D056 (partially superseded — (A) stands, (B) recharacterized, ">100%" retracted), D029 (released breaking change), D042 (hold scope at the epic), D054 (`COST_MODEL.md` caveat routing), D055; `#595` (trace linker; PR A `#645` landed, PR B open), `#549` (docs caveats), `#143` (superseded), `#468` (trace-missing cohort); Anthropic sources: Messages API (stateless), Agent SDK cost tracking ("the `usage` field undercounts as soon as nesting occurs"), Prompt caching (0.1x read), Usage & Cost Admin API.

---

## D058: `dogfood` issue milestoning — shippable-artifact test; re-measurement floats on the label

**Date:** 2026-07-22
**Context:** The #590 dogfood-runner emits two structurally different issue kinds under one `dogfood` label: (a) **re-measurement / validation** tasks that gate on corpus-window availability — enough post-fix sessions must accrue before the measurement means anything (#513, #469, #558, #600, #601, #637); and (b) **bugs the corpus surfaced** — real defects with a code fix (#646 totalTokens ~15x understatement, #648 orphan/depth-≥2 traces, 30.7% of subagent tokens dropped). During the 2026-07-22 v0.11.0 curation, #513 and #469 were milestoned v0.12.0 as a *convergence device* to let the v0.11 loop close, not as a scheduling commitment — leaving an unresolved rule for when a `dogfood` issue earns a version milestone.

**Decisions:**
- **Milestone = shippable PyPI artifact bound to that release.** A `dogfood` issue earns a release-version milestone only when closing it puts shippable code in the release — a fix a `pip install agentfluent` user would see. This is the same discriminator #559 applied to release-loop tooling (no artifact → no version milestone); dogfood milestoning is a special case of it. **Provenance is not a scheduling signal.**
- **Dogfood-surfaced bugs are milestoned like any other bug**, by severity/priority (#646, #648 → v0.12.0, correctly). The `dogfood` label records origin; it may co-exist with `bug` for provenance but does not affect scheduling. A dogfood-surfaced bug need not carry the `dogfood` label at all — #646/#648 carry `bug` only, and their provenance is captured in the milestone description prose, which is sufficient.
- **Dogfood re-measurement / validation issues default to `dogfood` label only, no milestone — they float.** Their gate (corpus-window availability) is orthogonal to any release's feature work; they cannot be "done" by shipping code, so they cannot be a version's blocker.
- **Escalation path.** A re-measurement issue is pulled onto a milestone only when its verdict is a genuine go/no-go blocker for a feature shipping in that release. When the release closes, an unresolved measurement comes **back off** the milestone rather than riding forward. Milestoning a validation issue solely to converge a release loop is an artifact to reverse at the next curation, not a precedent to inherit.

**Rationale:** A version milestone is a claim that shipping the release resolves its contents. Re-measurement issues violate that claim — they resolve when data accrues, on their own clock, regardless of what ships — so parking them on a milestone silently inflates release scope and manufactures false blockers. Floating them on the `dogfood` label keeps them discoverable and groupable without coupling them to a cut date. Dogfood-surfaced bugs have the opposite property: they close on a code change, so they schedule identically to any bug. This preserves the #559 principle (milestone tracks shippable artifacts, not activity) while giving the dogfood corpus a durable, uncluttered home.

**Reference:** #513 (architect re-measure — float), #469 (per-turn ratio stub — float, add `dogfood` label), #646/#648 (dogfood-surfaced bugs — keep on v0.12.0), #559 (tooling-off-milestone precedent), D050 (#590 dogfood-runner), D057 (v0.12.0 cost-correctness scope). Governs future dogfood-runner output.

---

## D059: v0.12 scoping — cost correctness (the quantity half) as headline; all four Phase 2 cost-levers ship, closing epic #535

**Date:** 2026-07-22
**Context:** v0.11 shipped the pricing *rate* half (genai-prices base⊕overlay seam #547, date-aware plumbing) and, per D057(11), an explicit caveat that the composite `estimated_total_cost_usd` is no more accurate than v0.10 because the token *quantity* feeding it is wrong (D057: `totalTokens` is a deterministic single-turn context proxy → ~15x per-agent understatement; a separate link-coverage defect drops 30.7% of subagent tokens before they reach `session total_cost`). D057(10) named #646 the v0.12.0 headline. After the D058 dogfood-milestoning cleanup, v0.12.0 carried 14 open issues. Two scoping questions: (1) confirm the release shape around the quantity fix; (2) the specific call — do all four remaining Phase 2 cost-levers (#536 fast mode, #537 batch, #538 residency, #539 server-tool) ride v0.12, or do the low-live-impact ones defer to keep the release focused?

**Decisions:**
- **v0.12 headline is the quantity half (WS1): #646 (real per-agent spend from per-turn `usage` + cache-diluted `blended_rate` correction, no rename per D029), #648 (decompose + attribute + disclose the 30.7% dropped-token coverage), #595 PR B (multi-level linker: `parent_invocation_id` + `depth`, depth-≥2 probe, cycle guard).** Per D057(7) #646 must correct *both* quantity and rate — the 15.2x cost error exceeds the 5.8x token error because both compound. The only hard sequencing blocks in the milestone: #648 AC2 ← #595 PR B, and #548 ← (#646 + #648).
- **All four Phase 2 cost-levers ship in v0.12 (#536/#537/#538/#539), closing epic #535.** They are small, independent overlay multipliers on the already-shipped #547 seam. Epic #535's bar is completeness (implement *or* document a limitation) — for a 1.1x residency multiplier, implementing is cheaper than writing the deferral prose and carrying the epic open. "$0 in our corpus" is explicitly rejected by the epic as the scheduling criterion (a published PyPI tool prices levers real for *some* user). No marquee dilution, because the headline is unambiguously the quantity fix. Rejected: (a) defer #538/#537 to v0.13 — strands the epic for a trivial multiplier, no focus benefit; (b) defer the whole Phase 2 batch — leaves the levers idle on a seam already paid for in v0.11.
- **Pricing-adjacent #543 (diff pricing-version stamp) and #82 (genai-prices pin currency + coverage detection) land**, both unblocked now Phase 1 shipped. **Hygiene #649/#652/#653 land.** **#548 docs catch-up sequences LAST**, hard-blocked on #646+#648: it must update the v0.11 cost caveat and add the v0.12.0 runtime caveat D057(11) deferred from v0.11.
- **Explicitly OUT:** #639 (>200K tier gap — real per D054 but a separate story, maintainer call, not part of the quantity/lever story); dogfood re-measurement #513/#469/#558/#600/#601/#637 (float on the `dogfood` label per D058 — orthogonal to shipping code). #646/#648 stay: dogfood-*surfaced bugs* carrying `bug`, shipping a fix a `pip install` user sees.

**Rationale:** The release cleanly splits along the same seam D057 drew: v0.11 got price-per-token right, v0.12 gets token-count right, so the composite cost number finally means what it says. Keeping the rate-lever completion in the same release is not a second marquee — it is the rate half of one cost-correctness story, sitting on infrastructure already built, and it retires an epic rather than opening one. The sizing (14, mid-band) and the completeness bar both argue for keeping all four levers rather than fewer: deferral shrinks the release below band and leaves #535 dangling across a release boundary for a 1.1x multiplier. **Validation caveat:** #646's independent reconciliation against the Usage & Cost Admin API is **blocked** — AgentFluent has no org/admin credentials (Fred, 2026-07-22), and per D057(15) that reconciliation was already scoped as a one-time *manual* check, never a merge AC. So #646/#648 validation rests on the internal cross-checks (production-parser agreement 80/80, CodeFluent corpus within 0.13%, D057(9)); the co-shipped levers being ~$0 in the dogfood corpus keeps any dogfood composite delta attributable to #646/#648 without needing the Admin API.

**Reference:** milestone v0.12.0 (14 issues); WS1 #646/#648/#595 (PR A shipped v0.11, PR B open); epic #535 Phase 2 #536/#537/#538/#539; adjacent #543/#82; hygiene #649/#652/#653; docs #548; spec `prd-v0.12.md`; governing D057 (headline + quantity/rate split + runtime-caveat deferral), D056 (aggregation convention (A) stands), D058 (dogfood milestoning), D054 (tier split to #639 + COST_MODEL caveat routing), D042 (hold scope at epic), D029 (released field → no rename), D047 (docs graduated route). Admin-API reconciliation blocked on org/admin creds (Fred, 2026-07-22) → internal-validation only. Out: #639, dogfood re-measurement #513/#469/#558/#600/#601/#637.

---

## D060: The loop-plugin (#611/#613) is named as **loop engineering**, not graph engineering — placement against the mid-2026 public terminology

**Date:** 2026-07-23
**Context:** Before scaffolding the plugin repo (#613/S2), Fred asked to settle the public terminology first, since the repo/marketplace name binds the `/plugin marketplace add <owner/repo>` path and every `${CLAUDE_PLUGIN_ROOT}` reference and is costly to change later. A newly-coined term, **"graph engineering,"** is circulating as the "next layer after loop engineering," and we needed to place our release-loop skill against both before naming.

**Terminology findings (public, mid-2026):**
- **Loop engineering** — the established term (popularized ~June 2026 by Addy Osmani, synthesizing Boris Cherny/Anthropic + Peter Steinberger). Unit = *one agent's repeating cycle*: build the system that finds work → does it → verifies it → remembers it, minimizing human-in-the-loop.
- **Graph engineering** — emerging (crystallized ~July 18 2026, Steinberger), explicitly "the layer that comes next." Used in **two distinct senses** that are often conflated: (1) *orchestration-topology* (LangGraph/AutoGen/Google-ADK lineage) — multiple specialized agent-nodes wired by explicit edges/routers/human-checkpoints with fan-out/fan-in; "loops make agent *behavior* programmable, graphs make agent *organizations* programmable"; (2) *governance / improvement-graph* (eigent.ai) — many feedback loops (metrics, evals, audits, policies) wired into a governed network that watch/constrain/correct each other, anchored to external ground truth with paired counter-metrics against Goodhart drift.

**Placement of our release-loop** (aibuilderclub 4-question gut-check; 0–1 = loop, 2–3 = genuine composition, 4 = paradigm shift): (1) separate specialized contexts? **yes** — `SCOPE_AGENT`/`DESIGN_AGENT`/fresh-context AC-verifier/parallel code-review finders; (2) real fan-out/fan-in? **partial** — code-review parallel finders + verify only; rest is sequential; (3) readable upfront control flow? **strongly yes** — explicit numbered pipeline (steps 0–12) + explicit router (§7.3) + explicit gates, the opposite of emergent; (4) changed success definition? **partial** — per-route/per-AC + human gate within one "ship the backlog" objective. **Score ≈ 2.5 → "genuine composition."**

**Decisions:**
- **Our skill is a *structured, routed loop* at the sophisticated end of loop engineering — NOT graph engineering** in either strict sense. Not the topology sense (single supervised orchestrator, **one issue per invocation**, sequential — not a concurrent agent org); not the governance sense (our eval/anchor loops — dogfood-runner #590, the anti-false-green gate, AC-as-held-out-spec, ledger-state-outside-context — genuinely exist but are **sibling loops, not wired into the release-loop as a governed graph**). One-liner: *it's a loop, not a graph-of-loops — but it's the kind of well-instrumented, explicitly-routed loop that graph engineering treats as a **node**.*
- **The name is loop-framed, and this is future-compatible, not future-obsolete.** Because graph engineering = "graph of loops," a clean loop-primitive is exactly the reusable node a future graph composes. A **"graph"-framed name would overclaim** — the engine is not a topology orchestrator — and would age worse than the honest loop framing.
- **Repo/marketplace name: `claude-code-loop` (CONFIRMED — Fred, 2026-07-23).** Naming principle applied: loop-framed + **Claude-Code-specific** (the artifact is not runtime-portable — `SKILL.md`/`/plugin`/`${CLAUDE_PLUGIN_ROOT}` are all Claude Code surfaces), and **no "harness"** (the word has drifted to mean the *model-execution wrapper*, which this is not; it's the loop *engine/orchestrator*). `claude-code-loop` chosen over `claude-loop-plugin` (redundant `-plugin`, doesn't match family) and `loop-harness` (rejected: not Claude-specific + loose "harness"); it mirrors sibling **`claude-code-sessions`** exactly. Full repo path: **`frederick-douglas-pearce/claude-code-loop`**. **Installable *plugin id* = `dev-loop` (CONFIRMED — Fred, 2026-07-23; decided during #613 scaffold, no longer deferred to #614).** `release-loop` is a historical artifact of the loop's origin working release milestones (v0.10/v0.11); the engine is milestone-agnostic (`BACKLOG_SOURCE` is a parameter) and does general supervised *development*, so `dev-loop` names the function, not the birthplace. Plugin id (`plugin.json` `name`) ≠ repo/marketplace name (`claude-code-loop`) — install string is `/plugin install dev-loop@claude-code-loop`. The **source skill in agentfluent stays named `release-loop`** (dir/frontmatter/`/release-loop`/drift-guard) until the port (#614) + dogfood swap (#616) rename it along. Supersedes the provisional `loop-harness` placeholder in D051.

**Rationale:** Getting the frame right before scaffolding avoids a costly rename of the marketplace path + `${CLAUDE_PLUGIN_ROOT}` references, and prevents the name from writing a conceptual check the artifact doesn't cash. "Loop" is both currently-correct and forward-compatible: our engine is precisely the node-shaped primitive that graph engineering composes, so the loop framing survives the graph-engineering transition rather than being obsoleted by it.

**Reference:** epic #611, story #613 (S2 scaffold), D051 (plugin distribution — provisional `loop-harness` name superseded here); source `.claude/skills/release-loop/{SKILL.md,loop-engine.md}` + `.claude/loop.config.md`; public sources — Osmani/loop-engineering (eesel, ADTmag), graph-engineering (aibuilderclub 4-question gut-check, eigent.ai improvement-graph, TrueFoundry, explainx graphs-vs-loops debate). Installable plugin-id decision deferred to #614/S3.
