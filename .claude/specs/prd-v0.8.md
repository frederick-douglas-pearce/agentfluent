...
| Idle-gap threshold re-tuning (#454) regresses subtraction quality on dramatic gaps | Existing well-handled cases stop being subtracted | Regression guard in #454 ACs: the 12 "obviously-stuck" traces from notebook §11 (span > 10min AND biggest_gap > 50% of span) must continue to have at least one gap flagged as idle. `STUCK_PATTERN` signal is unaffected — it uses retry-attempt counting in `diagnostics/trace_signals.py:198` (`_STUCK_MIN_ATTEMPTS = 4`), not the idle-gap constants. If re-tuning proves harder than expected, #454 can slip to v0.8.1 — #453 alone prevents silently misleading durations. |
